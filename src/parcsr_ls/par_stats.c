...
HYPRE_Int hypre_BoomerAMGMatTimes(void* data)
{
   ...
   for (i = 0; i < num_levels - 1; i++)
   {
      ...
      int num_requests = nsends + nrecvs;
...
/*
      ...
      if (nrecvs)
      {
         recvbuf1 = (double*)malloc(rdispls[nrecvs]*sizeof(double));
      }
      ...
      for (j = 0; j < nrecvs; ++j)
      {
         ...
         hypre_MPI_Recv_init( recvbuf1 + vec_start, vec_len, MPI_DOUBLE,
      ...
      for (j = 0; j < nsends; ++j)
      {
         ...
         hypre_MPI_Send_init( sendbuf + vec_start, vec_len, MPI_DOUBLE,
                              ip, 0, comm, requests + nrecvs + j );
      }
      tfinal = MPI_Wtime() - t0;
      MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
      if (rank == 0) printf("Standard init time: %e\n", t0);


      // warmup exchange
      if (num_requests > 0) {
         hypre_MPI_Startall(num_requests, requests);
         hypre_MPI_Waitall(num_requests, requests, hypre_MPI_STATUSES_IGNORE);
      }
      MPI_Barrier(comm);
      t0 = MPI_Wtime();
      if (num_requests > 0) {
      ...
      tfinal = MPI_Wtime() - t0;
      MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
      if (rank == 0) printf("Standard start/wait time: %e\n", t0/timing_iters);

      // warmup exchange
      if (num_requests > 0) {
         hypre_ParCSRPersistentCommHandleStart(comm_handle, HYPRE_MEMORY_HOST, sendbuf);
         hypre_ParCSRPersistentCommHandleWait(comm_handle, HYPRE_MEMORY_HOST, recvbuf1);
      }
      MPI_Barrier(comm);
      t0 = MPI_Wtime();
      if (num_requests > 0) {
...
/*
      ...
      for (j = 0; j < nsends; j++)
         sendcounts[j] = sdispls[j+1] - sdispls[j];
      for (j = 0; j < nrecvs; j++)
         recvcounts[j] = rdispls[j+1] - rdispls[j];
      ...
      tfinal = MPI_Wtime() - t0;
      MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0, comm);
      if (rank == 0) printf("unoptimized init time: %e\n", t0);
      ...
      for (j = 0; j < rdispls[nrecvs]; j++) {
          if (recvbuf1[j] != recvbuf2[j]) {
              pass_loc = false;
              hypre_printf("rank %d expected %f, got %f\n", rank, recvbuf1[j], recvbuf2[j]);
          }
      }
      ...
      // Partially Optimized Neighborhood Collective
      if (nsends)
         sendcounts = (int*)malloc(nsends*sizeof(int));
      if (nrecvs)
         recvcounts = (int*)malloc(nrecvs*sizeof(int));
      ...
      // partial optimized Neighbor Alltoallv Init Time
      MPI_Barrier(comm);
      t0 = MPI_Wtime();
      ...
      MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0,
            hypre_ParCSRCommPkgComm(comm_pkg));
      if (rank == 0) printf("Neighbor part nap Alltoallv Init Time %e\n", t0);
...
/*
      ...
      MPI_Reduce(&tfinal, &t0, 1, MPI_DOUBLE, MPI_MAX, 0,
            hypre_ParCSRCommPkgComm(comm_pkg));
      if (rank == 0) printf("full nap Neighbor Alltoallv Init Time %e\n", t0);
